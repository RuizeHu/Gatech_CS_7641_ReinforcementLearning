...
def FrozenLake():
	...
	for gamma in gamma_list:
		st=time.time()
		value_converged, n_iter = value_iteration(env, gamma)
		policy = get_policy(env, value_converged, gamma)
		end=time.time()
		score = get_score(env, policy, gamma)
		value.append(value_converged)
		score_list.append(score)
		num_iters.append(n_iter)
		time_list.append(end-st)
		
		plot_policy_map('Value Iteration '+ 'Gamma: ' + str(round(gamma,2)), policy.reshape(4,4), desc,colors_lake(), directions_lake(), value_converged)

	plt.plot(gamma_list, time_list)
	plt.xlabel('Gamma')
	plt.title('Run Time - Value Iteration')
	...
	plt.title('Reward - Value Iteration')
	plt.grid()
	plt.show()
	...
	plt.title('Convergence - Value Iteration')
	plt.grid()
	plt.show()
